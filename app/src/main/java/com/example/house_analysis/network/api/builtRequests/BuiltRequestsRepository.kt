...
class BuiltRequestsRepository {
    ...
    suspend fun login(userInfo: UserLoginData): Boolean {
        return withContext(backgroundThread.coroutineContext) {
            networkPostRepo.login(userInfo)
        }
    }

    suspend fun register(userInfo: UserRegisterData): Int {
        return withContext(backgroundThread.coroutineContext) {
            networkPostRepo.registration(userInfo)
        }
    }

    suspend fun getTasks(): ArrayList<TasksResponse> {
        return withContext(backgroundThread.coroutineContext) {
            networkGetRepo.getTasks()
        }
    }
